...
int main(void) {
	...
	// 1. Turn on HSE (this is configured as default when the MCU is reset)
	*p_rcc_cr_addr |= (1 << 16);

	// 2. Wait until HSE crystal is stablilized
	while (!(*p_rcc_cr_addr & (1 << 17)))
		;

	// 2. Set clock source MCO1 using HSI (Microcontroller clock output 1)
	*p_rcc_cfgr_addr &= ~(3 << 21);
	*p_rcc_cfgr_addr |= (2 << 21);

	// 2.1 Set clock MCO1 pre-scaler: division by 5
	*p_rcc_cfgr_addr |= (7 << 24);
